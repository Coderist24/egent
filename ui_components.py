...
def clean_message_content(content: str) -> str:
    """Clean message content from unwanted HTML tags"""
    import re
    
    if not isinstance(content, str):
        return str(content)
    
    # Remove div tags with message-related classes that might have been accidentally included
    content = re.sub(r'<div[^>]*class=["\'][^"\'>]*message[^"\'>]*["\'][^>]*>.*?</div>', '', content, flags=re.DOTALL | re.IGNORECASE)
    
    # Remove any remaining HTML div tags that look suspicious
...
def format_message_with_references(content: str) -> str:
    """Format message content to display references with special styling and clean HTML"""
    import re
    
    # First, clean any unwanted HTML tags that might have been accidentally included
    # Remove div tags with class "message-time" and similar unwanted HTML
    content = re.sub(r'<div[^>]*class=["\']message-time["\'][^>]*>.*?</div>', '', content, flags=re.DOTALL)
    content = re.sub(r'<div[^>]*class=["\']message-bubble["\'][^>]*>.*?</div>', '', content, flags=re.DOTALL)
    
    # IMPORTANT: Check if content already has formatted document references from Azure utils
    # If it does, don't process again to avoid double formatting and breaking
    if '<span class="document-reference">' in content:
        return content
    
    # Also check for existing document reference patterns that might be from Azure AI Agent
    if '📄' in content and any(ext in content.lower() for ext in ['.pdf', '.docx', '.doc', '.txt', '.xlsx']):
        # Don't return early, let it process to ensure consistent formatting
        pass
    
    # If no pre-formatted references, proceed with our formatting
    formatted_content = content
    
    # Pattern 1: Match old [referans: filename.ext] format
    reference_pattern1 = r'\[referans:\s*([^\]]+)\]'
    
    def replace_reference1(match):
        filename = match.group(1).strip()
        return f'<span class="document-reference">📄 {filename}</span>'
    
    # Apply old reference format first
    formatted_content = re.sub(reference_pattern1, replace_reference1, formatted_content)
    
    # Pattern 2: Find all complete filenames with better Turkish character support
    # More comprehensive pattern that handles Turkish characters and complex filenames
    complete_filename_pattern = r'\b[A-Za-z0-9_\-\.öçşğüıÖÇŞĞÜİıİçÇşŞğĞüÜöÖ]+\.(?:pdf|docx|doc|txt|xlsx|xls|ppt|pptx|csv|json|xml)\b'
    
    # Find all matches and sort by length (longest first) to prevent partial replacements
    all_filenames = re.findall(complete_filename_pattern, formatted_content, re.IGNORECASE)
    unique_filenames = list(set(all_filenames))  # Remove duplicates
    unique_filenames.sort(key=len, reverse=True)  # Sort by length, longest first
    
    # Additional pattern for common problematic filenames we see in debug
    # These are specifically from your debug output
    additional_known_files = [
        'ABAC_KULLANICI_SORUMLULUKLARI.docx',
        'IDRRU13_1.billisiyet_kontrolleri_Calisma_Talimat.docx',
        'ABAC_KULLANICI_SORUMLULKLARI.docx'  # variant spelling
    ]
    
    # Check if any of these known files appear in content
    for known_file in additional_known_files:
        if known_file in formatted_content and known_file not in unique_filenames:
            unique_filenames.append(known_file)
    
    # Replace each filename only if it's not already wrapped
    for filename in unique_filenames:
        # Check if this filename is already wrapped
        if f'<span class="document-reference">' not in filename and filename not in formatted_content.replace(filename, f'<span class="document-reference">📄 {filename}</span>'):
            # Use word boundary to ensure complete match, but be more flexible
            pattern = r'(?<!\w)' + re.escape(filename) + r'(?!\w)'
            replacement = f'<span class="document-reference">📄 {filename}</span>'
                    ...
                    if hasattr(ai_client, 'download_file_content'):
                        ...
                        if file_content and isinstance(file_content, bytes) and len(file_content) > 0:
                            ...
                            st.download_button(
                                ...
                                file_name=filename,
                                mime="application/octet-stream",
                                key=f"direct_download_{file_id}_{idx}",
...
def show_agent_chat(agent_config: Dict):
    ...
    for i, message in enumerate(agent_messages):
        ...
        if message["role"] == "user":
            formatted_content = format_message_with_references(clean_content)
            st.markdown(f"""
            ...
            # Check for downloadable files in the message
            client = st.session_state.ai_clients.get(agent_id)
            if client:
            ...
            if message.get("code_snippet"):
                with st.expander("🐍 View Python Code"):
                    st.code(message["code_snippet"], language="python")
    ...
    # Test button for document reference styling
    col1, col2 = st.columns([3, 1])
    
    user_input = st.chat_input(f"Ask {agent_config['name']}...", key=f"chat_{agent_id}")
    
    if user_input:
        # Add user message (clean it first)
        if agent_id not in st.session_state.messages:
            st.session_state.messages[agent_id] = []
        
        clean_user_input = clean_message_content(user_input)
        st.session_state.messages[agent_id].append({"role": "user", "content": clean_user_input})
        ...
        try:
            with st.spinner(f"🤔 {agent_config['name']} is thinking..."):
                ...
                # Check if this is a code interpreter request (contains keywords like "chart", "graph", "plot", "visualize")
                ...
                if is_code_request and hasattr(client, 'send_message_with_code_interpreter'):
                    # Add currency formatting instruction for Turkish charts
                    ...
                    assistant_message = {
                        "role": "assistant", 
                        "content": validated_response_text,
            ...
            # Check for downloadable files in the new message and display them immediately
            client = st.session_state.ai_clients.get(agent_id)
            if client and assistant_message.get("content"):
                display_downloadable_files(assistant_message["content"], client)
            
        except Exception as e:
            error_msg = f"Error getting response from Azure AI Foundry: {str(e)}"
            st.error(error_msg)
            # Clean error message before saving
            clean_error_msg = clean_message_content(error_msg)
            st.session_state.messages[agent_id].append({"role": "assistant", "content": clean_error_msg})
