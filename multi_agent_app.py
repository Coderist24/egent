...
class UserManager:
    ...
    def __init__(self, config: Optional[AzureConfig] = None):
        ...
        self.blob_user_manager = BlobStorageUserManager(self._config)
...
def initialize_session_state():
    ...
    if AZURE_AVAILABLE:
        ...
        agents = {
            'demo_agent': {
                ...
                'enabled': True,
                'send_user_info': False  # Yeni alan: Kullanıcı bilgilerini prompt'a ekle?
            }
        }
                ...
                for agent_id, config in all_agents.items():
                    if config.get('enabled', True):
                        ...
                        agents[agent_id] = {
                            ...
                            'send_user_info': config.get('send_user_info', False)
                        }
    ...
    try:
        if AZURE_AVAILABLE:
            ...
            if not user_manager:
                azure_config = st.session_state.get('azure_config') or AzureConfig()
                st.session_state['azure_config'] = azure_config
                user_manager = UserManager(azure_config)
