...
# Timezone utilities for consistent Turkey time handling
...
def get_turkey_datetime_string():
    """Get current Turkey time as ISO string"""
    return get_turkey_time().isoformat()

def parse_turkey_time(time_string):
    """Parse ISO time string and convert to Turkey timezone"""
    try:
        if time_string:
            ...
            # If naive datetime, assume it's already in Turkey time
...
class AzureConfig:
    ...
    def __init__(self):
        ...
        self.client_id = os.getenv("AZURE_CLIENT_ID", "a9f24ac4-bb75-4c9b-81b7-fdfadfd4fb80")
        self.client_secret = os.getenv("AZURE_CLIENT_SECRET", "CQy8Q~W9m1kJQywMg88zYlB4CFLi40lPjyOyxa8z")
        ...
        self.ai_project_name = os.getenv("AZURE_AI_PROJECT_NAME", "ege-egentproject")
        self.resource_group_name = os.getenv("AZURE_RESOURCE_GROUP", "egentai") 
        ...
        # Optional fallback (eski çalışan) servis principal bilgileri
        # Eğer yeni app ile 403 alırsak bunları kullanıp yeniden deneriz
        self.alt_client_id = os.getenv(
            "ALT_AZURE_CLIENT_ID",
            os.getenv("OLD_AZURE_CLIENT_ID", "706af514-7245-4658-9f36-1f79071a80f6")
        )
        self.alt_client_secret = os.getenv(
            "ALT_AZURE_CLIENT_SECRET",
            os.getenv("OLD_AZURE_CLIENT_SECRET", "Ddo8Q~vJSZoAnmMMEz.zKR4n_6usxpnqvOV-ibNR")
        )
        self.alt_tenant_id = os.getenv("ALT_AZURE_TENANT_ID", self.tenant_id)
    ...
    def get_agent_credential(self, attempt_fallback: bool = False):
        """Credential specifically for Agent (AI Project) çağrıları.

        attempt_fallback True ise alternatif (eski) SP ile dener.
        """
        ...
        try:
            return ClientSecretCredential(
                ...
                client_secret=self.alt_client_secret
            )
        except Exception:
            return self.get_credential()
...
class EnhancedAzureAIAgentClient:
    ...
    def __init__(self, connection_string: str, agent_id: str, config: AzureConfig, container_name: str = None):
        ...
        # Auto-discover agent if not resolved
        if self.client and not self.agent:
            self._auto_discover_agent()

    def _auto_discover_agent(self):
        """If given agent_id not found, pick first available agent and set it."""
        try:
            agents = []
            try:
                resp = self.client.agents.list_agents()
            ...
            if agents:
                picked = agents[0]
                try:
                    self.agent = self.client.agents.get_agent(picked.id)
                    self.agent_id = picked.id
                except Exception:
                    pass
        except Exception:
            pass
                ...
                if self.connection_string and self.agent_id and self.connection_string.strip():
                    ...
                    if not clean_conn_str or clean_conn_str.startswith('#'):
                        ...
                        if all([subscription_id, resource_group, project_name]):
                            ...
                            # 1. Yeni (current) credential ile dene
                            last_error = None
                            for attempt, use_fallback in enumerate([False, True]):
                                try:
                                    credential = self.config.get_agent_credential(attempt_fallback=use_fallback)
                                    ...
                                    # Basit ping: list_agents -> 403 ise fallback tetiklenecek
                                    try:
                                        self.client.agents.list_agents()
                                        # Başarılıysa loop kır
                                        break
                                    except Exception as ping_err:
                                        if "403" in str(ping_err) or "Forbidden" in str(ping_err):
                                            # İlk deneme ve yetki hatası -> fallback'e geç
                                            if not use_fallback:
                                                last_error = ping_err
                                                self.client = None
                                                continue
                                        # Diğer hatalar için de fallback dene
                                        if not use_fallback:
                                            last_error = ping_err
                                            self.client = None
                                            continue
                                        # Fallback de başarısızsa çık
                                        last_error = ping_err
                                    ...
                                    if not use_fallback:
                                        continue
                                # If here after fallback attempt without break -> failure
                            if not self.client:
                                self.agent = None
                                return
                        else:
                        ...
                        if self.client:
                            try:
                                # Agent'i doğrudan çek
                                self.agent = self.client.agents.get_agent(self.agent_id)
                            except Exception as agent_get_error:
                                # Eğer yetki 403 ise ve fallback kullanılmadıysa tek sefer daha fallback credential ile dene
                                if ("403" in str(agent_get_error) or "Forbidden" in str(agent_get_error)):
                                    try:
                                        fb_cred = self.config.get_agent_credential(attempt_fallback=True)
                                        fb_client = AIProjectClient(
                                            endpoint="https://eastus2.api.azureml.ms",
                                            subscription_id=self.config.subscription_id,
                                        ...
                                        self.agent = fb_client.agents.get_agent(self.agent_id)
                                        if self.agent:
                                            # Fallback client'i kalıcı yap
                ...
                try:
                    ...
                    if hasattr(agent_info, 'tool_resources') and agent_info.tool_resources:
                        ...
                        if file_search:
                            ...
                            elif hasattr(file_search, 'vector_store_ids'):
    ...
    def send_message_with_code_interpreter(self, thread_id: str, user_message: str) -> tuple[str, Optional[str], Optional[str]]:
        ...
        try:
            ...
            # Check for available files and add context
            # For Data Analyzer agents with job system, skip file validation and use job-based approach
            available_files_context = ""
            file_access_instructions = ""
            
            # Check if this is a Data Analyzer agent (job-based file management)
            agent_name = getattr(self.agent, 'name', '')
            is_data_analyzer = any(keyword in agent_name.lower() for keyword in ['analyzer', 'analiz'])
            
            if is_data_analyzer:
                # For Data Analyzer agents, provide job-based file access instructions
                available_files_context = "\n\n[Job sistemi ile dosyalar yüklenmiştir]"
                file_access_instructions = """

DOSYA ERİŞİM TALİMATLARI (Job Sistemi):
1. İlk olarak mevcut dosyaları listele: 
   import os; print("Çalışma dizini:", os.getcwd()); print("Dosyalar:", os.listdir('.'))

2. Excel dosyasını bul ve oku:
   import pandas as pd
   # Çalışma dizinindeki Excel dosyalarını bul
   import glob
   excel_files = glob.glob('*.xlsx') + glob.glob('*.xls')
   if excel_files:
       df = pd.read_excel(excel_files[0])
       print(f"Dosya yüklendi: {excel_files[0]}")
   else:
       print("Excel dosyası bulunamadı")

3. Analiz yapmaya başla - dosyalar job sistemi ile yüklenmiştir.
"""
            else:
                # Original file checking for non-Data Analyzer agents
                try:
                    # Agent'daki dosyaları kontrol et
                    agent_file_ids = []
                    if hasattr(self.agent, 'tool_resources') and self.agent.tool_resources:
                        if hasattr(self.agent.tool_resources, 'code_interpreter') and self.agent.tool_resources.code_interpreter:
                            if hasattr(self.agent.tool_resources.code_interpreter, 'file_ids'):
                                agent_file_ids = list(self.agent.tool_resources.code_interpreter.file_ids or [])
                    
                    # Her dosya ID'si için detay bilgi al
                    file_details = []
                    for file_id in agent_file_ids:
                        try:
                            file_info = self.client.agents.get_file(file_id)
                            file_details.append({
                                'id': file_id,
                                'filename': getattr(file_info, 'filename', 'Unknown'),
                                'size': getattr(file_info, 'size', 'Unknown')
                            })
                        except Exception as file_detail_error:
                            logger.warning(f"Dosya detayı alınamadı {file_id}: {file_detail_error}")
                    
                    if file_details:
                        file_list = [f['filename'] for f in file_details]
                        available_files_context = f"\n\nMevcut dosyalar: {', '.join(file_list)}"
                        
                        # Dosya erişim talimatları
                        file_access_instructions = f"""

DOSYA ERİŞİM TALİMATLARI:
1. İlk olarak mevcut dosyaları listele: 
   import os; print("Çalışma dizini:", os.getcwd()); print("Dosyalar:", os.listdir('.'))

2. Dosyayı bul ve oku:
   Ana dosya: {file_list[0] if file_list else 'Dosya bulunamadı'}
   
3. Pandas ile oku:
   import pandas as pd
   df = pd.read_excel('{file_list[0] if file_list else 'dosya.xlsx'}')
   
4. Eğer dosya yoksa çalışma dizinini ve dosyaları kontrol et.
"""
                    else:
                        available_files_context = "\n\nUYARI: Agent'da hiç dosya bulunamadı."
                        file_access_instructions = """

DOSYA ERİŞİM KONTROL TALİMATLARI:
1. Çalışma dizinini kontrol et: import os; print("Dizin:", os.getcwd())
2. Dosya listesini göster: print("Dosyalar:", os.listdir('.'))
3. Eğer dosya yoksa kullanıcıya bildir.
"""
                        
                except Exception as file_check_error:
                    logger.error(f"Dosya kontrol hatası: {file_check_error}")
                    available_files_context = "\n\nDosya kontrol hatası oluştu."
                    file_access_instructions = "\n\nDosya listesini kontrol et: import os; print(os.listdir('.'))"            # Enhance user message with Turkish currency formatting instructions if needed
            enhanced_message = user_message + available_files_context + file_access_instructions
            ...
            try:
                if hasattr(self, '_pending_file_ids') and self._pending_file_ids:
                    run_tool_resources = {"code_interpreter": {"file_ids": list(self._pending_file_ids)}}
            except Exception as run_tr_err:
                logger.debug(f"Run tool_resources hazırlama hatası: {run_tr_err}")

            if run_tool_resources:
                try:
                    run = self.client.agents.create_and_process_run(
                        thread_id=thread_id,
                        agent_id=self.agent.id,
                ...
                except Exception as run_with_tr_err:
                    logger.warning(f"Run-level tool_resources başarısız: {run_with_tr_err}, tool_resources olmadan denenecek")
                    run = self.client.agents.create_and_process_run(
                        thread_id=thread_id,
                        agent_id=self.agent.id
                    )
            ...
            # DETAYLI DEBUG: Dosya eklerini kontrol et
            logger.info("Dosya ekleri kontrol ediliyor...")
            # Check for file annotations (images) - Updated for new API
            if hasattr(messages, 'data') and messages.data:
                for message in messages.data:
                    if message.role == "assistant" and hasattr(message, 'content'):
                        logger.info(f"Assistant mesajında {len(message.content)} content item var")
                        for j, content_item in enumerate(message.content):
                            logger.info(f"Content item {j} tipi: {type(content_item)}")
                            logger.info(f"Content item {j} özellikleri: {dir(content_item)}")
                            # Check for image file references
                            if hasattr(content_item, 'image_file'):
                                logger.info("Image file bulundu!")
                                try:
                                    file_id = content_item.image_file.file_id
                                    file_name = f"agent_image_{thread_id[-8:]}.png"
                                    logger.info(f"Image dosyası kaydediliyor: {file_name}, File ID: {file_id}")
                                    ...
                                    logger.info(f"Image başarıyla kaydedildi: {image_path}")
                                    break
                                    
                                except Exception as img_error:
                                    logger.error(f"Image kaydetme hatası: {img_error}")
                        if image_path:
                            break
            ...
            # Fallback: Check for file annotations (old API format)            
                ...
                logger.info("Eski API formatı dosya eklerini kontrol ediliyor...")
                try:
                    ...
                    image_path = file_name
                    logger.info(f"Eski API ile image kaydedildi: {image_path}")
                    
                except Exception as img_error:
                    logger.error(f"Eski API image hatası: {img_error}")
            
            # DETAYLI DEBUG: Python kod parçacığını çıkar
            logger.info("Python kod parçacığı aranıyor...")
            ...
            try:
                ...
                logger.info(f"Run steps sayısı: {len(getattr(run_details, 'data', []))}")
                
                for i, step in enumerate(run_details.data):
                    logger.info(f"Step {i}: Type={getattr(step.step_details, 'type', 'Type yok')}")
                    if getattr(step.step_details, 'type', None) == "tool_calls":
                        logger.info(f"Tool calls bulundu - Toplam: {len(step.step_details.tool_calls)}")
                        for j, call in enumerate(step.step_details.tool_calls):
                            logger.info(f"Tool call {j}: Has code_interpreter={hasattr(call, 'code_interpreter')}")
                            if hasattr(call, 'code_interpreter') and call.code_interpreter:
                                ...
                                if input_value:
                                    ...
                                    logger.info(f"Python kodu bulundu - uzunluk: {len(code_snippet)}")
                                    break
                        if code_snippet:
                            break
                            
            except Exception as code_error:
                logger.error(f"Kod çıkarma hatası: {code_error}")
                import traceback
                logger.error(f"Code extraction full error: {traceback.format_exc()}")
            
            logger.info(f"Result - Response: {len(response_text) if response_text else 0} characters, "
                      f"Image: {'Available' if image_path else 'None'}, "
                      f"Code: {'Var' if code_snippet else 'Yok'}")

            # Fallback: if no image_path captured but code/interpreter likely generated a figure, search local dir
            if image_path is None:
                try:
                    import glob, os
                    candidate_images = glob.glob('agent_image_*.png') or glob.glob('*.png')
                    # Filter out very small placeholder files (<2 KB)
                    candidate_images = [p for p in candidate_images if os.path.getsize(p) > 2048]
                    if candidate_images:
                        # Pick most recently modified
                        candidate_images.sort(key=lambda p: os.path.getmtime(p), reverse=True)
                        image_path = candidate_images[0]
                        logger.info(f"Fallback image selected: {image_path}")
                except Exception as fallback_err:
                    logger.debug(f"Fallback image arama hatası: {fallback_err}")
            
            return response_text, image_path, code_snippet
            
        except Exception as e:
            logger.error(f"send_message_with_code_interpreter genel hatası: {str(e)}")
            import traceback
            logger.error(f"Tam hata detayı: {traceback.format_exc()}")
            return f"Error: {str(e)}", None, None

    def upload_blob_file_to_agent(self, container_name: str, file_name: str) -> Optional[str]:
        """
        Download file from blob storage and upload to AI agent for code interpreter
        ...
        try:
            ...
            # Storage connection kontrolü
            if not self.config.storage_connection_string:
                logger.error("Storage connection string yapılandırılmamış")
                return None
            
            # Download file from blob storage
            blob_service_client = BlobServiceClient.from_connection_string(self.config.storage_connection_string)
            blob_client = blob_service_client.get_blob_client(container=container_name, blob=file_name)
            
            # Check if blob exists first
            try:
                blob_properties = blob_client.get_blob_properties()
                logger.info(f"Blob found - Size: {blob_properties.size} bytes")
            except Exception as blob_check_error:
                logger.error(f"Blob access error: {blob_check_error}")
                ...
                try:
                    ...
                    if not blobs:
                        logger.warning("Container empty or no files found")
                except Exception as container_error:
                    logger.error(f"Container access error {container_name}: {container_error}")
                    return None
                
                # Try to find a similar filename with multiple normalization strategies
                found_blob = None
                
                # Strategy 1: Exact match (case insensitive)
                for blob in blobs:
                    if blob.name.lower() == file_name.lower():
                        found_blob = blob
                        break
                
                # Strategy 2: Normalize spaces/underscores/hyphens
                if not found_blob:
                    normalized_target = file_name.lower().replace(' ', '_').replace('-', '_')
                    for blob in blobs:
                        normalized_blob = blob.name.lower().replace(' ', '_').replace('-', '_')
                        if normalized_blob == normalized_target:
                            found_blob = blob
                            break
                
                # Strategy 3: Remove all spaces and special characters
                if not found_blob:
                    import re
                    clean_target = re.sub(r'[^a-zA-Z0-9.]', '', file_name.lower())
                    for blob in blobs:
                        clean_blob = re.sub(r'[^a-zA-Z0-9.]', '', blob.name.lower())
                        if clean_blob == clean_target:
                            found_blob = blob
                            break
                
                # Strategy 4: Partial match on filename without extension
                if not found_blob:
                    target_base = file_name.lower().split('.')[0]
                    target_ext = '.' + file_name.lower().split('.')[-1] if '.' in file_name else ''
                    
                    for blob in blobs:
                        blob_base = blob.name.lower().split('.')[0]
                        blob_ext = '.' + blob.name.lower().split('.')[-1] if '.' in blob.name else ''
                        
                        # Normalize and compare base names
                        norm_target_base = re.sub(r'[^a-zA-Z0-9]', '', target_base)
                        norm_blob_base = re.sub(r'[^a-zA-Z0-9]', '', blob_base)
                        
                        if norm_target_base == norm_blob_base and target_ext == blob_ext:
                            found_blob = blob
                            break
                
                if found_blob:
                    logger.info(f"Similar file found: {found_blob.name}, using instead of {file_name}")
                    blob_client = blob_service_client.get_blob_client(container=container_name, blob=found_blob.name)
                    file_name = found_blob.name  # Update filename for logging
                    try:
                        blob_properties = blob_client.get_blob_properties()
                        logger.info(f"Using file: {file_name}, Size: {blob_properties.size} bytes")
                    except Exception:
                        logger.error(f"Cannot access found file: {found_blob.name}")
                        return None
                else:
                    logger.error(f"No similar file found for {file_name} in container: {container_name}")
                    return None
            
            # Download blob content
            blob_data = blob_client.download_blob().readall()
            logger.info(f"{file_name} downloaded from container {container_name}, size: {len(blob_data)} bytes")
            ...
            # Upload to AI agent using correct SDK method
            try:
                file_result = self.client.agents.upload_file(
                    file=file_stream,
                    purpose="assistants"
                )
                ...
                if file_result and hasattr(file_result, 'id'):
                    ...
                    logger.info(f"{file_name} successfully uploaded to AI agent - file_id: {file_id}")
                    
                    # Verify the uploaded file is accessible
                    max_retries = 3
                    for attempt in range(max_retries):
                        try:
                            verification_file = self.client.agents.get_file(file_id)
                            ...
                            if attempt < max_retries - 1:
                                ...
                                logger.error(f"File verification failed: {file_id}")
                                return file_id
                    
                    return file_id
                else:
                    logger.error(f"{file_name} upload failed - no file_id returned")
                    return None
                    
            except Exception as upload_error:
                logger.error(f"Upload_file method failed: {upload_error}")
                return None
                
        except Exception as e:
            logger.error(f"Blob file upload to agent general error: {str(e)}")
            return None

    def create_code_interpreter_agent(self, model: str, name: str, instructions: str, data_container: str = None, data_file: str = None, clear_existing: bool = True) -> bool:
        """
        Create a temporary agent with code interpreter capabilities and optionally load data file
        
        Args:
            model: AI model to use
            name: Agent name
            instructions: Agent instructions
            data_container: Azure blob container name (optional)
            data_file: File name in container (optional)
            clear_existing: If True, clears existing files before adding new one
        """
        try:
            if not self.client:
                logger.error("AI client not initialized")
            ...
            # Upload data file if specified
            file_ids = []
            if data_container and data_file:
                try:
                    file_id = self.upload_blob_file_to_agent(data_container, data_file)
                    if file_id:
                        file_ids.append(file_id)
                        logger.info(f"File successfully uploaded - File ID: {file_id}")
                    else:
                        logger.warning(f"File upload failed: {data_file}")
                except Exception as upload_error:
                    logger.error(f"File upload error {data_file}: {upload_error}")

            # Attach file to existing agent
            if file_ids:
                try:
                    success = self._attach_file_to_agent(self.agent_id, file_ids[0], clear_existing=clear_existing)
                    if success:
                        logger.info(f"File successfully attached to agent: {file_ids[0]}")
                    else:
                        logger.warning("File could not be attached to agent")
                except Exception as attach_error:
                    logger.error(f"File attachment error: {attach_error}")

            return True
                
        except Exception as e:
            logger.error(f"create_code_interpreter_agent error: {str(e)}")
            return False

    def _attach_file_to_agent(self, agent_id: str, file_id: str, clear_existing: bool = False) -> bool:
        """
        ...
        """
        try:
            logger.info(f"[ATTACH DEBUG] Starting _attach_file_to_agent - Agent: {agent_id}, File: {file_id}, Clear existing: {clear_existing}")
            
            # Azure AI model sınıflarını import et
            ...
            # Önce dosyanın gerçekten var olduğunu kontrol et
            try:
                file_info = self.client.agents.get_file(file_id)
                logger.info(f"File verified: {file_info.filename if hasattr(file_info, 'filename') else file_id}")
            except Exception as file_check_error:
                logger.error(f"❌ Dosya bulunamadı: {file_id} - {file_check_error}")
                return False
            
            # Agent'ı al ve mevcut tools/resources'ı analiz et
            current_agent = self.client.agents.get_agent(agent_id)
            
            # Mevcut Code Interpreter file IDs'ları al
            current_code_interpreter_files = []
            existing_vector_store_ids = []
            existing_knowledge_file_ids = []
            existing_azure_ai_search_resources = None
            
            if hasattr(current_agent, 'tool_resources') and current_agent.tool_resources:
                # Code Interpreter dosyalarını al
                ...
                # Knowledge (File Search) vector store'ları ve file'ları al ve koru
                if hasattr(current_agent.tool_resources, 'file_search') and current_agent.tool_resources.file_search:
                    ...
                    # File search ile ilişkili file_ids da olabilir - bunlar da Knowledge dosyaları
                    if hasattr(current_agent.tool_resources.file_search, 'file_ids') and current_agent.tool_resources.file_search.file_ids:
                        existing_knowledge_file_ids = list(current_agent.tool_resources.file_search.file_ids or [])
                        logger.info(f"Found {len(existing_knowledge_file_ids)} Knowledge files to preserve")
                
                # Azure AI Search kaynaklarını al ve koru
                if hasattr(current_agent.tool_resources, 'azure_ai_search') and current_agent.tool_resources.azure_ai_search:
                    existing_azure_ai_search_resources = current_agent.tool_resources.azure_ai_search
                    logger.info("Found existing Azure AI Search resources to preserve (includes hr-index)")
            
            # Mevcut tools'ları kontrol et ve Knowledge tool'unu koru
            current_tools = []
            has_file_search = False
            has_azure_ai_search = False
            if hasattr(current_agent, 'tools') and current_agent.tools:
                for tool in current_agent.tools:
                    if hasattr(tool, 'type'):
                        current_tools.append(tool.type)
                        if tool.type == 'file_search':
                            has_file_search = True
                            logger.info("Found existing file_search tool - will preserve")
                        elif tool.type == 'azure_ai_search':
                            has_azure_ai_search = True
                            logger.info("Found existing azure_ai_search tool - will preserve")
                        elif tool.type == 'code_interpreter':
                            logger.info("Found existing code_interpreter tool")
                    
            # Sadece Code Interpreter dosyalarını temizle (Knowledge dosyalarına dokunma)
            if clear_existing and current_code_interpreter_files:
                logger.info(f"Clearing {len(current_code_interpreter_files)} code interpreter files")
                try:
                    for old_file_id in current_code_interpreter_files:
                        try:
                            # Sadece gerçekten code interpreter dosyası olduğunu kontrol et
                            self.client.agents.delete_file(old_file_id)
                            logger.info(f"Code interpreter file deleted: {old_file_id}")
                        except Exception as delete_error:
                            logger.warning(f"Could not delete code interpreter file {old_file_id}: {delete_error}")
                    current_code_interpreter_files = []  # Code interpreter listesini temizle
                except Exception as cleanup_error:
                    logger.warning(f"Code interpreter file cleanup error: {cleanup_error}")
            
            # Dosya zaten ekliyse ve temizlik yapmadıysak
            if not clear_existing and file_id in current_code_interpreter_files:
                logger.info(f"File already attached to agent: {file_id}")
                return True
            
            # Yeni Code Interpreter file ID'yi ekle
            if clear_existing:
                new_code_interpreter_files = [file_id]  # Sadece yeni dosya
            else:
                new_code_interpreter_files = current_code_interpreter_files + [file_id]  # Mevcut + yeni
            
            logger.info(f"New code interpreter files: {len(new_code_interpreter_files)} files")
            
            # Tool resources oluştur - Knowledge dosyalarını koru
            code_interpreter_resource = CodeInterpreterToolResource(file_ids=new_code_interpreter_files)
            updated_tool_resources = ToolResources(code_interpreter=code_interpreter_resource)
            
            # Knowledge vector store'ları ve file search'i koru
            if existing_vector_store_ids or has_file_search or existing_knowledge_file_ids:
                # Vector store IDs ve Knowledge file IDs'ları ile file search resource oluştur
                file_search_resource = FileSearchToolResource(
                    vector_store_ids=existing_vector_store_ids,
                    file_ids=existing_knowledge_file_ids
                )
                updated_tool_resources.file_search = file_search_resource
                logger.info(f"Preserved Knowledge: {len(existing_vector_store_ids)} vector stores + {len(existing_knowledge_file_ids)} files")
                
                # Azure AI Search indexlerini de koruma için ek log
                if existing_vector_store_ids:
                    logger.info(f"Knowledge indexleri korundu: {existing_vector_store_ids}")
                if existing_knowledge_file_ids:
                    logger.info(f"Knowledge dosyaları korundu: {existing_knowledge_file_ids}")
            
            # Azure AI Search kaynaklarını koru
            if existing_azure_ai_search_resources:
                updated_tool_resources.azure_ai_search = existing_azure_ai_search_resources
                logger.info("Preserved Azure AI Search resources (hr-index protected)")
            
            # Tools listesini oluştur - mevcut tools'ları da koru
            tools = [CodeInterpreterToolDefinition()]
            
            # Mevcut Azure AI Search tool'unu koru
            if has_azure_ai_search:
                # Azure AI Search tool'unu doğru formatta ekle
                try:
                    tools.append(AzureAISearchToolDefinition())
                    logger.info("Preserved existing azure_ai_search tool")
                except Exception as azure_search_error:
                    # Fallback için generic tool oluştur
                    logger.warning(f"Could not create AzureAISearchToolDefinition: {azure_search_error}")
                    azure_ai_search_tool = type('AzureAISearchTool', (), {'type': 'azure_ai_search'})()
                    tools.append(azure_ai_search_tool)
                    logger.info("Preserved existing azure_ai_search tool (fallback)")
            
            # File search tool'unu ekle veya koru
            if has_file_search or existing_vector_store_ids or existing_knowledge_file_ids:
                tools.append(FileSearchToolDefinition())
                logger.info("Added/preserved file_search tool for Knowledge functionality (includes Azure AI Search indexes)")
            
            # Agent'ı güncelle - Knowledge dosyalarını ve indexleri koruyarak, agent name'ini de koru
            updated_agent = self.client.agents.update_agent(
                agent_id=agent_id,
                name=current_agent.name,  # Agent name'ini koru
                tools=tools,
                tool_resources=updated_tool_resources
            )
            
            logger.info(f"Agent successfully updated: {updated_agent.id}")
            logger.info(f"Preserved Knowledge: {len(existing_vector_store_ids)} vector stores, {len(existing_knowledge_file_ids)} files")
            
            # Güncelleme sonrası kısa bekleme (senkronizasyon için)
            time.sleep(1)
            
            # Güncellemeyi birkaç kez doğrula (retry mekanizması)
            max_retries = 3
            for attempt in range(max_retries):
                try:
                    verification_agent = self.client.agents.get_agent(agent_id)
                    ...
                    if file_id in verification_file_ids:
                        logger.info(f"File successfully attached to agent: {file_id}")
                        
                        # Ekstra doğrulama: Dosyaya erişim testi
                        try:
                            # Bir test thread oluşturup dosyanın erişilebilir olduğunu kontrol et
                            test_thread = self.client.agents.create_thread()
                            test_message = self.client.agents.create_message(
                                thread_id=test_thread.id,
                                role="user",
                                content="Bu dosyada hangi kolonlar var?"
                            )
                            
                            # Thread'i temizle
                            self.client.agents.delete_thread(test_thread.id)
                            logger.info(f"File access test successful")
                            
                        except Exception as access_test_error:
                            logger.warning(f"File access test error: {access_test_error}")
                        
                        return True
                    else:
                        if attempt < max_retries - 1:
                            logger.warning(f"File not yet verified, retrying... ({attempt + 1}/{max_retries})")
                            time.sleep(2)  # Biraz daha bekle
                        else:
                            logger.error(f"File not found in verification: {file_id}")
                            return False
                            
                except Exception as verification_error:
                    if attempt < max_retries - 1:
                        logger.warning(f"Verification error, retrying: {verification_error}")
                        time.sleep(2)
                    else:
            ...
            logger.error(f"❌ Dosya agent'a eklenirken hata: {e}")
        ...
        try:
            if self.client and self.agent and hasattr(self.agent, 'id'):
                ...
                if 'code-interpreter' in getattr(self.agent, 'name', '').lower():
                    logger.info(f"Cleaning up temporary agent: {self.agent.id}")
                    self.client.agents.delete_agent(self.agent.id)
                    
                    # Restore original agent
                    if hasattr(self, '_original_agent') and self._original_agent:
                        self.agent = self._original_agent
                        logger.info("Restored original agent")
                    elif self.agent_id:
                        try:
                            self.agent = self.client.agents.get_agent(self.agent_id)
                            logger.info(f"Restored agent from ID: {self.agent_id}")
                        except Exception as restore_error:
                            logger.warning(f"Could not restore original agent: {restore_error}")
...
class BlobStorageUserManager:
    ...
    def _hash_password(self, password: Optional[str]) -> str:
        """Hash password using SHA256; gracefully handle empty/None passwords.

        For Azure AD backed users we may not store a local password. In that case we
        persist an empty string so downstream auth that expects a hash simply fails
        (non-admin) without throwing exceptions.
        """
        try:
            if not password:
                return ""  # No local password (Azure AD or passwordless user)
    ...
    def create_admin_user(self, username: str, password: str) -> bool:
        ...
        try:
            ...
            # Normalize username by trimming leading/trailing spaces
            ...
            blob_name = f"{normalized_username}.json"
            blob_client = self.blob_client.get_blob_client(
                container=self.container_name,
                blob=blob_name
            )
    ...
    def get_user_permissions(self, username: str) -> Dict:
        ...
        try:
            # Normalize username by trimming spaces
            normalized_username = (username or "").strip()
            user_data = self.get_user(normalized_username)
            if user_data:
                return {
    ...
    def add_user(self, username: str, role: str, password: Optional[str], permissions: List[str] = None) -> bool:
        """Add new user. Password may be None for Azure AD / passwordless accounts."""
        try:
            if not self.blob_client:
                return False
            ...
            if permissions is None:
                permissions = self._get_user_permissions(normalized_username, role)
            
            user_data = {
                'username': normalized_username,
                'password_hash': self._hash_password(password),  # empty string if None
                'role': role,
                'permissions': permissions,
                'created_at': datetime.now().isoformat(),
                'updated_at': datetime.now().isoformat()
            ...
            # Swallow exception but return False; could add logging here if logger available
            return False
...
class AgentManager:
    ...
    def __init__(self, config: AzureConfig):
        ...
        self._agents_cache: Optional[Dict[str, Dict]] = None
        self._agents_cache_timestamp: float = 0.0

    def _invalidate_cache(self):
        self._agents_cache = None
        self._agents_cache_timestamp = 0.0

    def _ensure_cache(self, force_refresh: bool = False):
        try:
            if force_refresh or self._agents_cache is None or (time.monotonic() - self._agents_cache_timestamp) > self.CACHE_TTL_SECONDS:
                agents = self.blob_agent_manager.get_all_agents()
                if not isinstance(agents, dict):
                    agents = {}
                self._agents_cache = agents
                self._agents_cache_timestamp = time.monotonic()
        except Exception:
            # On any error, fall back to empty cache to avoid propagating failures
            self._agents_cache = {}
            self._agents_cache_timestamp = time.monotonic()
    ...
    def get_agent(self, agent_id: str) -> Optional[Dict]:
        ...
        try:
            self._ensure_cache()
            agent_map = self._agents_cache or {}
    ...
    def get_all_agents(self, force_refresh: bool = False) -> Dict[str, Dict]:
        """Get all agent configurations"""
        self._ensure_cache(force_refresh=force_refresh)
        return dict(self._agents_cache or {})
    
    def get_active_agents(self, force_refresh: bool = False) -> Dict[str, Dict]:
        """Get only active agent configurations"""
        agents = self.get_all_agents(force_refresh=force_refresh)
        return {
    ...
    def add_agent(self, agent_config: Dict) -> bool:
        ...
        result = self.blob_agent_manager.add_agent(agent_config)
        if result:
    ...
    def save_agent(self, configuration: Dict) -> Dict:
        ...
        try:
            ...
            try:
                ...
                result = {"status": "success", "location": "local", "id": configuration['id'], "action": action}
                self._invalidate_cache()
                return result
    ...
    def update_agent(self, agent_id: str, agent_config: Dict) -> bool:
        ...
        result = self.blob_agent_manager.update_agent(agent_id, agent_config)
        if result:
            self._invalidate_cache()
        return result
    ...
    def delete_agent(self, agent_id: str) -> bool:
        ...
        result = self.blob_agent_manager.delete_agent(agent_id)
        if result:
            self._invalidate_cache()
        return result
    ...
    def set_agent_status(self, agent_id: str, status: str) -> bool:
        """Set agent status (active/inactive)"""
        result = self.blob_agent_manager.set_agent_status(agent_id, status)
        if result:
            self._invalidate_cache()
        return result
...
class UserManager:
    ...
    def __init__(self, config: Optional[AzureConfig] = None):
        ...
        self.blob_user_manager = BlobStorageUserManager(self.config)
        self._permissions_cache: Dict[str, Dict] = {}
        self._permissions_cache_timestamp: Dict[str, float] = {}
        self._all_users_cache: Optional[Dict[str, Dict]] = None
        self._all_users_cache_timestamp: float = 0.0

    def _invalidate_permissions_cache(self, username: Optional[str] = None):
        if username:
            normalized = (username or "").strip()
            self._permissions_cache.pop(normalized, None)
            self._permissions_cache_timestamp.pop(normalized, None)
        else:
            self._permissions_cache.clear()
            self._permissions_cache_timestamp.clear()
        self._all_users_cache = None
        self._all_users_cache_timestamp = 0.0

    def _get_cached_permissions(self, username: str) -> Dict:
        normalized = (username or "").strip()
        now = time.monotonic()
        cached = self._permissions_cache.get(normalized)
        cached_ts = self._permissions_cache_timestamp.get(normalized, 0.0)
        if cached is not None and (now - cached_ts) <= self.CACHE_TTL_SECONDS:
            return cached

        permissions = self.blob_user_manager.get_user_permissions(normalized)
        if not isinstance(permissions, dict):
            permissions = {'permissions': [], 'role': 'guest'}
        self._permissions_cache[normalized] = permissions
        self._permissions_cache_timestamp[normalized] = now
        return permissions
    ...
    def get_user_permissions(self, username: str) -> Dict:
        """Get user permissions"""
        return self._get_cached_permissions(username)
    ...
    def has_permission(self, username: str, agent_id: str, permission_type: str) -> bool:
        ...
        user_permissions = self._get_cached_permissions(normalized_username)
        ...
        if isinstance(permissions, dict):
            ...
            if permission_type in permissions:
                return True
            specific_permission = f"{agent_id}:{permission_type}"
            return specific_permission in permissions
        return False
    ...
    def add_user(self, username: str, role: str, password: str, permissions: List[str] = None) -> bool:
        ...
        result = self.blob_user_manager.add_user(username, role, password, permissions or [])
        if result:
            self._invalidate_permissions_cache(username)
        return result
    
    def update_user_permissions(self, username: str, permissions: List[str]) -> bool:
        """Update user permissions"""
        result = self.blob_user_manager.update_user_permissions(username, permissions)
        if result:
            self._invalidate_permissions_cache(username)
        return result
    ...
    def delete_user(self, username: str) -> bool:
        """Delete a user"""
        result = self.blob_user_manager.delete_user(username)
        if result:
            self._invalidate_permissions_cache(username)
        return result
    ...
    def get_all_users(self) -> Dict[str, Dict]:
        """Get all users"""
        now = time.monotonic()
        if self._all_users_cache is None or (now - self._all_users_cache_timestamp) > self.CACHE_TTL_SECONDS:
            users = self.blob_user_manager.get_all_users()
            if not isinstance(users, dict):
                users = {}
            self._all_users_cache = users
            self._all_users_cache_timestamp = now
        return dict(self._all_users_cache or {})

# Job Management System for Data Analyzer Agents
class JobManager:
    """Manages background jobs for Data Analyzer agents"""
    
    def __init__(self, config: AzureConfig):
        self.config = config
        self.container_name = "job-configs"
        self.logs_container = "job-logs"
        self.blob_client = None
    ...
    def _init_blob_client(self):
        ...
        try:
            ...
            try:
                self.blob_client.create_container(self.container_name)
                self.blob_client.create_container(self.logs_container)
            except Exception as e:
                pass  # Containers might already exist
                
        except Exception as e:
            logger.error(f"JobManager blob client initialization failed: {e}")
            self.blob_client = None
    
    def create_job(self, job_config: Dict) -> bool:
        """Create a new job configuration"""
        try:
            if not self.blob_client:
                return False
            
            job_id = job_config.get('id') or f"job_{get_turkey_time().strftime('%Y%m%d_%H%M%S')}"
            job_config['id'] = job_id
            job_config['created_at'] = get_turkey_datetime_string()
            job_config['updated_at'] = get_turkey_datetime_string()
            job_config['status'] = 'created'
            
            # Validate required fields for all agent job types
            required_fields = ['agent_id', 'agent_type', 'schedule_type', 'data_container', 'data_files']
            for field in required_fields:
                if field not in job_config:
                    logger.error(f"Missing required field: {field}")
                    return False
            
            # Allow jobs for all agent types (Data Agent, Data Analyzer, etc.)
            logger.info(f"Creating job for agent type: {job_config.get('agent_type')}")
            ...
            blob_client.upload_blob(
                json.dumps(job_config, indent=2).encode('utf-8'),
                overwrite=True
            )
            
            # Send notification to admin users
            self._send_job_notification(job_id, 'created', 'Job successfully created')
    ...
    def get_jobs_for_agent(self, agent_id: str) -> List[Dict]:
        ...
        try:
            ...
            for blob in container_client.list_blobs():
                if blob.name.endswith('.json'):
                    ...
                    if job_data and job_data.get('agent_id') == agent_id:
                        jobs.append(job_data)
            
            return jobs
            
        except Exception as e:
            logger.error(f"Error getting jobs for agent {agent_id}: {e}")
            return []
    ...
    def update_job(self, job_id: str, job_config: Dict) -> bool:
        ...
        try:
            ...
            # Get existing config to preserve creation time
            existing_config = self.get_job(job_id)
            if existing_config:
                job_config['created_at'] = existing_config.get('created_at')
    ...
    def delete_job(self, job_id: str) -> bool:
        ...
        try:
            ...
            # Send notification
            self._send_job_notification(job_id, 'deleted', 'Job deleted')
    ...
    def execute_job(self, job_id: str) -> bool:
        """Execute a job manually or via scheduler"""
        try:
            ...
            job_config['last_run'] = get_turkey_datetime_string()
            self.update_job(job_id, job_config)
            
            # Log job start
            self._log_job_event(job_id, 'started', 'Job execution started')
            
            # Send notification
            self._send_job_notification(job_id, 'started', 'Job execution started')
            
            try:
                # Execute the actual job
                success = self._execute_agent_job(job_config)
                
                if success:
                    job_config['status'] = 'completed'
                    job_config['last_success'] = get_turkey_datetime_string()
                    self._log_job_event(job_id, 'completed', 'Job completed successfully')
                    self._send_job_notification(job_id, 'completed', 'Job completed successfully')
                else:
                    job_config['status'] = 'failed'
                    job_config['last_failure'] = get_turkey_datetime_string()
                    self._log_job_event(job_id, 'failed', 'Job execution failed')
                    self._send_job_notification(job_id, 'failed', 'Job execution failed')
                ...
                logger.error(f"Job execution error {job_id}: {execution_error}")
                job_config['status'] = 'failed'
                job_config['last_failure'] = get_turkey_datetime_string()
                job_config['last_error'] = str(execution_error)
                self.update_job(job_id, job_config)
                
                self._log_job_event(job_id, 'failed', f'Job execution error: {str(execution_error)}')
                self._send_job_notification(job_id, 'failed', f'Job execution error: {str(execution_error)}')
    ...
    def _execute_agent_job(self, job_config: Dict) -> bool:
        """Execute agent job - upload files to code interpreter (works for all agent types)"""
        try:
            agent_id = job_config.get('agent_id')
            data_container = job_config.get('data_container')
            data_files = job_config.get('data_files', [])
            
            if not all([agent_id, data_container, data_files]):
                logger.error("Missing required job configuration")
                return False
            
            # Initialize Azure AI client for the agent
            try:
                # Get agent configuration from BlobStorageAgentManager
                blob_agent_manager = BlobStorageAgentManager(self.config)
                agent_config = blob_agent_manager.get_agent(agent_id)
                
                if not agent_config:
                    logger.error(f"Agent configuration not found: {agent_id}")
                    return False
                
                # Debug logging for scheduled vs manual execution
                logger.info(f"[JOB DEBUG] Agent ID: {agent_id}")
                logger.info(f"[JOB DEBUG] Agent Config Keys: {list(agent_config.keys())}")
                logger.info(f"[JOB DEBUG] Connection String length: {len(agent_config.get('connection_string', ''))}")
                logger.info(f"[JOB DEBUG] Container Name: {agent_config.get('container_name', '')}")
                
                # Initialize EnhancedAzureAIAgentClient
                ...
                # Add new files to code interpreter (clear old code interpreter files, preserve Knowledge)
                logger.info(f"Adding new files to agent {agent_id} (clearing old code interpreter files, preserving Knowledge files & indexes)")
                
                # Upload each specified file
                uploaded_files = []
                for file_name in data_files:
                    logger.info(f"Uploading file: {file_name} from container: {data_container}")
                    
                    file_id = ai_client.upload_blob_file_to_agent(data_container, file_name)
                    if file_id:
                        uploaded_files.append(file_id)
                        logger.info(f"Successfully uploaded: {file_name} -> {file_id}")
                        
                        # Attach file to agent (clear code interpreter files on first file only, always preserve Knowledge)
                        success = ai_client._attach_file_to_agent(
                            agent_config.get('agent_id', ''), 
                            file_id, 
                            clear_existing=(len(uploaded_files) == 1)  # Clear code interpreter files only on first file
                        )
                        
                        if not success:
                            logger.warning(f"Failed to attach file to agent: {file_name}")
                    else:
                        logger.error(f"Failed to upload file: {file_name}")
                        return False
                
                if uploaded_files:
                    logger.info(f"Job completed successfully. Uploaded {len(uploaded_files)} files")
                    return True
                else:
                ...
                logger.error(f"AI client error in job execution: {ai_error}")
                return False
                
        except Exception as e:
            logger.error(f"Error in _execute_agent_job: {e}")
            return False
        ...
        try:
            if job_id in self.running_jobs:
                ...
                if job_config:
                    ...
                    job_config['stopped_at'] = get_turkey_datetime_string()
                    self.update_job(job_id, job_config)
                
                self._log_job_event(job_id, 'stopped', 'Job stopped manually')
                self._send_job_notification(job_id, 'stopped', 'Job stopped manually')
    ...
    def _log_job_event(self, job_id: str, event_type: str, message: str):
        """Log job event to blob storage"""
        try:
            if not self.blob_client:
                return
            ...
            log_entry = {
                ...
                'message': message,
                'timestamp': get_turkey_datetime_string()
            }
            
            # Create log file name with timestamp
            timestamp = get_turkey_time().strftime('%Y%m%d_%H%M%S_%f')
            log_blob_name = f"{job_id}/{timestamp}_{event_type}.json"
            ...
            logger.error(f"Error logging job event: {e}")
    
    def get_job_logs(self, job_id: str, limit: int = 50) -> List[Dict]:
        """Get job logs from blob storage"""
        try:
            if not self.blob_client:
                return []
            
            container_client = self.blob_client.get_container_client(self.logs_container)
            logs = []
            
            # List blobs with job_id prefix
            for blob in container_client.list_blobs(name_starts_with=f"{job_id}/"):
                ...
                try:
                    ...
                    blob_data = blob_client.download_blob().readall()
                    log_entry = json.loads(blob_data.decode('utf-8'))
                    logs.append(log_entry)
                except Exception as log_error:
                    logger.warning(f"Error reading log {blob.name}: {log_error}")
    ...
    def _send_job_notification(self, job_id: str, event_type: str, message: str):
        """Send email notification to admin users"""
        try:
            # Get admin users
            user_manager = BlobStorageUserManager(self.config)
            all_users = user_manager.get_all_users()
            
            admin_emails = []
            for username, user_data in all_users.items():
                if user_data.get('role') == 'admin':
                    email = user_data.get('email') or f"{username}@company.com"
                    admin_emails.append(email)
            
            if not admin_emails:
                logger.warning("No admin users found for job notification")
                return
            
            # Create email content
            subject = f"Job {event_type.upper()}: {job_id}"
            body = f"""
Job Notification
================
...
This is an automated notification from the Multi-Agent Platform.
"""
            
            # Try to send email via Azure Communication Services
            email_sent = self._send_email_via_azure_communication_services(admin_emails, subject, body)
            
            if not email_sent:
                # Fallback: Log the notification
                logger.info(f"EMAIL NOTIFICATION - To: {', '.join(admin_emails)}")
    ...
    def _send_email_via_azure_communication_services(self, recipients: List[str], subject: str, body: str) -> bool:
        """Send email using Azure Communication Services"""
        try:
            # Azure Communication Services configuration
            # You need to set these environment variables:
            communication_connection_string = os.getenv("AZURE_COMMUNICATION_CONNECTION_STRING")
            sender_email = os.getenv("AZURE_COMMUNICATION_SENDER_EMAIL", "noreply@yourcompany.com")
            
            if not communication_connection_string:
                logger.warning("Azure Communication Services not configured - using fallback logging")
                return False
            ...
            try:
                ...
                logger.warning("Azure Communication Services SDK not installed - using fallback logging")
                return False
            except Exception as email_error:
                logger.error(f"Azure Communication Services error: {email_error}")
                return False
    ...
    def get_job_status(self, job_id: str) -> Dict:
        ...
        try:
            ...
            status = {
                'id': job_id,
                'status': job_config.get('status', 'unknown'),
                'created_at': job_config.get('created_at'),
                'updated_at': job_config.get('updated_at'),
                'last_run': job_config.get('last_run'),
                'last_success': job_config.get('last_success'),
                'last_failure': job_config.get('last_failure'),
                'last_error': job_config.get('last_error')
            }
            
            # Add runtime info if job is currently running
            if job_id in self.running_jobs:
                runtime_info = self.running_jobs[job_id]
                status['running_since'] = runtime_info['start_time'].isoformat()
                status['current_status'] = runtime_info['status']
...
# Job Scheduler for Background Jobs
class JobScheduler:
    ...
    def _scheduler_loop(self):
        ...
        while self.running:
            try:
                self._check_and_execute_scheduled_jobs()
                time.sleep(60)  # Check every minute
            except Exception as e:
                logger.error(f"Scheduler loop error: {e}")
                time.sleep(60)
    
    def _check_and_execute_scheduled_jobs(self):
        """Check for jobs that need to be executed"""
        try:
            current_time = get_turkey_time()  # Use Turkey time
            ...
            for job in all_jobs:
                if (job.get('schedule_type') == 'scheduled' and 
                    job.get('status') != 'running' and
                    job.get('schedule_hour') == current_hour and
                    job.get('schedule_minute') == current_minute):
                    
                    # Check schedule period
                    schedule_period = job.get('schedule_period', 'daily')
                    should_execute = False
                    
                    if schedule_period == 'daily':
                        # Check if job was already run today
                        should_execute = not self._was_run_today(job, current_time)
                    
                    elif schedule_period == 'weekly':
                        # Check if it's the right day of week and not run this week
                        job_weekday = job.get('schedule_weekday', 'Monday')
                        should_execute = (current_weekday == job_weekday and 
                                        not self._was_run_this_week(job, current_time))
                    
                    elif schedule_period == 'monthly':
                        # Check if it's the right day of month and not run this month
                        job_day = job.get('schedule_day', 1)
                        should_execute = (current_day == job_day and 
                                        not self._was_run_this_month(job, current_time))
                    
                    else:
                        # Fallback to daily for backward compatibility
                        should_execute = not self._was_run_today(job, current_time)
                    
                    if should_execute:
                        logger.info(f"Executing scheduled job: {job['id']} ({schedule_period})")
                        self.job_manager.execute_job(job['id'])
                    
        except Exception as e:
            logger.error(f"Error checking scheduled jobs: {e}")
    
    def _was_run_today(self, job, current_time):
        """Check if job was already run today (Turkey time)"""
        last_run = job.get('last_run')
        if last_run:
            try:
                # Parse last run time and convert to Turkey timezone
                last_run_turkey = parse_turkey_time(last_run)
                if last_run_turkey:
                    return last_run_turkey.date() == current_time.date()
            except Exception:
                pass
        return False
    
    def _was_run_this_week(self, job, current_time):
        """Check if job was already run this week (Turkey time)"""
        last_run = job.get('last_run')
        if last_run:
            try:
                # Parse last run time and convert to Turkey timezone
                ...
                if last_run_turkey:
                    ...
                    return last_run_turkey.date() >= week_start.date()
            except Exception:
                pass
        return False
    
    def _was_run_this_month(self, job, current_time):
        """Check if job was already run this month (Turkey time)"""
        last_run = job.get('last_run')
        if last_run:
            try:
                # Parse last run time and convert to Turkey timezone
                last_run_turkey = parse_turkey_time(last_run)
                if last_run_turkey:
                    return (last_run_turkey.year == current_time.year and 
                           last_run_turkey.month == current_time.month)
            except Exception:
                pass
        return False

# Global scheduler instance (will be initialized when needed)
_global_scheduler = None

def get_global_scheduler(job_manager: JobManager = None) -> JobScheduler:
    """Get or create global scheduler instance"""
    global _global_scheduler
    
    if _global_scheduler is None and job_manager is not None:
        _global_scheduler = JobScheduler(job_manager)
